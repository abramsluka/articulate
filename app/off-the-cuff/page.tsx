...
import { AuthRequiredError, QuotaExceededError, fetchWithSessionRefresh } from "../lib/api-client";
...
import type { Prompt, FilterCategory, TakeFilter } from "../data/prompts";
...
export default function Home() {
  ...
  const [analysisStatus, setAnalysisStatus] = useState<
    "idle" | "analyzing" | "done" | "error" | "requires-login" | "quota-exceeded"
  >("idle");
          ...
          try {
            ...
            if (typeof data.text === "string") {
              ...
              try {
                ...
                setAnalysisResult(null);
                setAnalysisStatus(
                  analysisError instanceof AuthRequiredError
                    ? "requires-login"
                    : analysisError instanceof QuotaExceededError
                      ? "quota-exceeded"
                      : "error"
                );
              }
            ...
            setTranscriptionStatus("error");
            setAnalysisStatus(
              transcriptionError instanceof AuthRequiredError
                ? "requires-login"
                : transcriptionError instanceof QuotaExceededError
                  ? "quota-exceeded"
                  : "error"
            );
            setAnalysisResult(null);
            ...
            <div className="flex flex-col gap-3 rounded-lg border border-slate-800 bg-slate-900/70 p-2.5">
              {!isTranscriptionSupported ? (
                ...
                <>
                  ...
                  {analysisStatus === "quota-exceeded" ? (
                    <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4">
                      <p className="text-sm text-amber-100">
                        You&apos;ve reached today&apos;s practice limit. It resets at midnight UTC, so come back tomorrow.
                      </p>
                    </div>
                  ) : null}
